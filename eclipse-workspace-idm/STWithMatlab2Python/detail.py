...
class detail:
	...
	def __new__(cls):
		if not cls._instance:
			...
			noms_colonnes = ["Annee", "Recettes T1", "Recettes T2", "Recettes T3", "Dépenses T1", "Dépenses T2", "Dépenses T3", "Bilan T1", "Bilan T2", "Bilan T3", "Bilan", "Meilleur trimestre", "Rapport recettes/dépenses (%)"]
	...
	def insertFromTable(self):
		# Pour toutes les colonnes de self.table qui ont un attribut contenu de type "ImportTable"
			self.fusionner_colonnes(recettes.get_instance().table, "Annee", "Année", "T1", "Recettes T1")
			self.fusionner_colonnes(recettes.get_instance().table, "Annee", "Année", "T2", "Recettes T2")
			self.fusionner_colonnes(recettes.get_instance().table, "Annee", "Année", "T3", "Recettes T3")
			self.fusionner_colonnes(depenses.get_instance().table, "Annee", "Année", "T1", "Dépenses T1")
			self.fusionner_colonnes(depenses.get_instance().table, "Annee", "Année", "T2", "Dépenses T2")
			self.fusionner_colonnes(depenses.get_instance().table, "Annee", "Année", "T3", "Dépenses T3")
	...
	def calcAll(self):
		...
		if path[-3:] == ".py":
			for j,element in enumerate(self.table["Bilan T1"]):
				colonne_provisoire.append(self.load_fct("calcul", "Expression2Python/SoustractionBinaire.py")(self.table.at[j,'Recettes T1'], self.table.at[j,'Dépenses T1']))
		elif path[-2:] == ".m":
			for j,element in enumerate(self.table["Bilan T1"]):
				colonne_provisoire.append(self.load_matlab_fct(path)(self.table.at[j,'Recettes T1'], self.table.at[j,'Dépenses T1']))
		...
		if path[-3:] == ".py":
			for j,element in enumerate(self.table["Bilan T2"]):
				colonne_provisoire.append(self.load_fct("calcul", "Expression2Python/SoustractionBinaire.py")(self.table.at[j,'Recettes T2'], self.table.at[j,'Dépenses T2']))
		elif path[-2:] == ".m":
			for j,element in enumerate(self.table["Bilan T2"]):
				colonne_provisoire.append(self.load_matlab_fct(path)(self.table.at[j,'Recettes T2'], self.table.at[j,'Dépenses T2']))
		...
		if path[-3:] == ".py":
			for j,element in enumerate(self.table["Bilan T3"]):
				colonne_provisoire.append(self.load_fct("calcul", "Expression2Python/SoustractionBinaire.py")(self.table.at[j,'Recettes T3'], self.table.at[j,'Dépenses T3']))
		elif path[-2:] == ".m":
			for j,element in enumerate(self.table["Bilan T3"]):
				colonne_provisoire.append(self.load_matlab_fct(path)(self.table.at[j,'Recettes T3'], self.table.at[j,'Dépenses T3']))
		...
		# Charger dynamiquement la fonction à partir du fichier spécifié par le chemin
		if path[-3:] == ".py":
			for j,element in enumerate(self.table["Rapport recettes/dépenses (%)"]):
				colonne_provisoire.append(self.load_fct("calcul", "Expression2Python/RapportRecettesSurDepenses.py")(self.table.at[j,'Recettes T1'], self.table.at[j,'Recettes T2'], self.table.at[j,'Recettes T3'], self.table.at[j,'Dépenses T1'], self.table.at[j,'Dépenses T2'], self.table.at[j,'Dépenses T3']))
		elif path[-2:] == ".m":
			for j,element in enumerate(self.table["Rapport recettes/dépenses (%)"]):
				colonne_provisoire.append(self.load_matlab_fct(path)(self.table.at[j,'Recettes T1'], self.table.at[j,'Recettes T2'], self.table.at[j,'Recettes T3'], self.table.at[j,'Dépenses T1'], self.table.at[j,'Dépenses T2'], self.table.at[j,'Dépenses T3']))
		self.table["Rapport recettes/dépenses (%)"] = colonne_provisoire
